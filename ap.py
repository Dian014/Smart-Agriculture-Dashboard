...
with st.sidebar: st.checkbox("Dark Mode", value=st.session_state.dark_mode, key="dark_mode") st.markdown("Location (coordinates)")

Coordinates (global default at 0,0)

LAT = st.sidebar.number_input("Latitude", value=0.000000, format="%.6f") LON = st.sidebar.number_input("Longitude", value=0.000000, format="%.6f")

------------------ Header ------------------

st.title("Global Smart Agriculture Dashboard") st.markdown( """ This dashboard provides global, location-based weather insights, irrigation guidance, yield prediction, fertilizer planning, and commodity price tracking for both crops and plantation commodities. """ )
...
threshold = st.sidebar.slider("Irrigation Threshold (mm)", 0, 20, 5) df_daily["Irrigation Recommendation"] = df_daily["Precipitation (mm)"].apply( lambda x: "Irrigation Needed" if x < threshold else "Sufficient" )
...
st.download_button("Download CSV", csv, "daily_weather.csv", "text/csv")

excel_io = BytesIO()
with pd.ExcelWriter(excel_io, engine='xlsxwriter') as writer:
    df_daily.to_excel(writer, index=False, sheet_name="Daily Weather")
    workbook = writer.book
    worksheet = writer.sheets["Daily Weather"]
    date_format = workbook.add_format({'num_format': 'yyyy-mm-dd'})
    worksheet.set_column('A:A', 15, date_format)
excel_io.seek(0)
st.download_button("Download Excel", data=excel_io.read(), file_name="daily_weather.xlsx")

html_table = df_daily.to_html(index=False)
b64 = base64.b64encode(html_table.encode("utf-8")).decode("utf-8")
href = f'<a href="data:text/html;base64,{b64}" download="daily_weather.html">📥 Download Daily Weather (HTML)</a>'
...
st.subheader("Automatic Yield (Generic Commodity, default yields)")
commodities = [
    "Rice", "Maize (Corn)", "Wheat", "Soybean", "Coffee", "Cocoa",
    "Oil Palm", "Rubber", "Tea", "Cotton", "Cassava", "Potato"
]
commodity = st.selectbox("Select Commodity", commodities, key="generic_cmd")
default_yield = {
    "Rice": 5000,
    "Maize (Corn)": 6000,
    "Wheat": 4500,
...
yield_per_ha = float(default_yield.get(commodity, 5000))
area = st.number_input("Field Size (ha)", value=1.0, key="gen_area")
price = st.number_input(f"Price per kg for {commodity}", value=1.0, key="gen_price")
cost_per_ha = st.number_input("Production Cost per ha", value=1000.0, key="gen_cost")

total_yield = yield_per_ha * area
revenue = total_yield * price
profit = revenue - (cost_per_ha * area)
...
st.subheader("Automatic Rice Yield (from Daily Weather Averages)")
area_auto = st.number_input("Field Size (ha) (auto)", value=1.0, key="auto_area")
price_auto = st.number_input("Price per kg (auto)", value=1.0, key="auto_price")
cost_auto = st.number_input("Production Cost per ha (auto)", value=1000.0, key="auto_cost")

if not df_daily.empty:
    X_auto = df_daily[["Precipitation (mm)", "Max Temp (°C)", "Humidity (%)"]].mean().values.reshape(1, -1)
    pred_auto = float(model.predict(X_auto)[0])
else:
    pred_auto = 0.0

total_auto = pred_auto * area_auto
revenue_auto = total_auto * price_auto
profit_auto = revenue_auto - (cost_auto * area_auto)
...
st.markdown("*Annual Rice Projection (3 harvests)*")
df_h1 = df_daily.head(7)
x1 = df_h1[["Precipitation (mm)", "Max Temp (°C)", "Humidity (%)"]].mean().values.reshape(1, -1)
p1 = float(model.predict(x1)[0])

df_h2 = df_daily[60:67] if len(df_daily) >= 67 else df_daily.tail(7)
x2 = df_h2[["Precipitation (mm)", "Max Temp (°C)", "Humidity (%)"]].mean().values.reshape(1, -1)
p2 = float(model.predict(x2)[0])

df_h3 = df_daily[120:127] if len(df_daily) >= 127 else df_daily.tail(7)
x3 = df_h3[["Precipitation (mm)", "Max Temp (°C)", "Humidity (%)"]].mean().values.reshape(1, -1)
p3 = float(model.predict(x3)[0])

area_year = st.number_input("Field Size (ha) (annual)", value=1.0, key="annual_area")
price_year = st.number_input("Price per kg (annual)", value=1.0, key="annual_price")
cost_year = st.number_input("Production Cost per ha (annual)", value=1000.0, key="annual_cost")

t1 = p1 * area_year
t2 = p2 * area_year
t3 = p3 * area_year
total_y = t1 + t2 + t3
revenue_y = total_y * price_year
total_cost_y = cost_year * area_year * 3
profit_y = revenue_y - total_cost_y
...
st.success(f"🟩 Annual Total Yield: {total_y:,.0f} kg | Revenue: {revenue_y:,.2f}")
st.success(f"🟨 Annual Net Profit: {profit_y:,.2f}")
...
fert_reco = {
    ...
    "Coffee": {
        "NPK": {"dose": 500, "function": "Overall growth & fruiting"}
    },
    "Cocoa": {
    ...
    "Oil Palm": {
        "NPK": {"dose": 300, "function": "Growth & productivity"}
    },
    "Rubber": {
        "NPK": {"dose": 250, "function": "Vegetative growth"}
    },
    "Tea": {
        "NPK": {"dose": 200, "function": "Leaf growth & yield"}
    },
    "Cotton": {
        "NPK": {"dose": 250, "function": "Vegetative & boll set"}
    },
    "Cassava": {
        "NPK": {"dose": 200, "function": "Root development"}
    },
    "Potato": {
        "NPK": {"dose": 250, "function": "Tuber development"}
    },
}
...
PRICE_FILE = "data/commodity_prices.json"
...
st.markdown("---") st.caption("© 2025 – Global Agriculture Dashboard")
